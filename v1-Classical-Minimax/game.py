    ...
    def recurse(bd, r0, c0, path, visited):
        ...
        for dr,dc in dirs:
            ...
            if mid and mid.lower() != p.lower() and end is None:
                # make jump on a copied board
                bd2 = copy.deepcopy(bd)
                bd2[endr][endc] = bd2[r0][c0]
                bd2[r0][c0] = None
                bd2[midr][midc] = None
                # crown if reaches last row
...
def heuristic_uninformed(board, side):
    # simple piece difference (CPU is 'b')
    score = 0
    for r in board:
        for cell in r:
...
def parse_move(s):
    try:
        ...
        for p in parts:
            ...
            # support formats like 6c3 or 6 3? keep simple: rc where r is digit(s) and c is digit(s)
            # We support rcc where both digits may be multi-digit, but user will enter like 6 3 which we don't handle.
            # We'll accept rNcM like '6c3' or '62c31' is unlikely. Simpler: expect two numbers separated by a letter. Simpler still:
            # Instead, allow format r,c using comma.
        # Fallback to accepting pattern r1c2-r3c4
    except Exception:
        return None
    # Better to implement robust parser specifically for rNcM format
    return parse_move_strict(s)

import re

def parse_move_strict(s):
    # accepts: R1C2-R3C4 or r1c2-r3c4 where numbers can be 1..8
    s = s.strip().lower()
    tokens = s.split('-')
    coords = []
    for t in tokens:
